...
bus.on('编译插件', function(){
    ...
    return postobject.plugin(__filename, function(root, context){
        ...
        root.walk( 'Tag', (node, object) => {
            ...
            if ( ary.legnth > 1 ) {
                // 属性 @csslib 不能重复
                throw new Err('duplicate attribute of @csslib', {file: context.input.file, text: context.input.text, start: ary[1].object.loc.start.pos, end: ary[1].object.loc.end.pos});
            }
            if ( /^(if|for)$/.test(object.value) ) {
                throw new Err(`unsupport attribute @csslib on tag <${object.value}>`, {file: context.input.file, text: context.input.text, start: ary[0].object.loc.start.pos, end: ary[0].object.loc.end.pos});
            }
